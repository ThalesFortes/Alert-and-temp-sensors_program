...
void registrar_dados(uint16_t distancia_mm, const char* estado, uint64_t tempo_ms, AHT10_t *medida) {
    ...
    FRESULT fr = f_open(&arquivo, "dados.txt", FA_OPEN_APPEND | FA_WRITE);
    if (fr == FR_OK) {
        snprintf(linha, sizeof(linha),
                 "[%02lu:%02lu] Dist: %s %s | Estado: %s | Temp: %.1fC | Umid: %.1f%%\n",
                 minutos, segundos, valor_str, unidade, estado,
                 medida->temp_celsius, medida->umidade_relativa);
        UINT bytes_escritos;
...
int main() {
    ...
    if (!AHT10_inicializar(I2C_PORT)) {
        ...
        ssd1306_WriteString("AHT10 falhou!", Font_7x10, White);
        ssd1306_UpdateScreen();
        led_red();
        while (1) tight_loop_contents();
    }
    ...
    while (1) {
        ...
        // Leitura AHT10
        if (!AHT10_obterMedicao(I2C_PORT, &medida)) {
            medida.temp_celsius = -99.0f;
            medida.umidade_relativa = -1.0f;
        }
        ...
        snprintf(line, sizeof(line), "T:%.1fC U:%.1f%%", medida.temp_celsius, medida.umidade_relativa);
        ...
        registrar_dados(media, estado, tempo_ms, &medida);

        // Debug console
        printf("[%llu ms] %s - %u mm | Temp: %.1fC | Umid: %.1f%%\n",
               tempo_ms, estado, media, medida.temp_celsius, medida.umidade_relativa);
